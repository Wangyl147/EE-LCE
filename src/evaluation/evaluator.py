...
class MyEvaluatorEvent(EvaluatorBase):
    def _init_metric(self):
        self.metric = MetricAcc()
    def _extract(self, json_data, predict):
        y_truth = set()
        for event in self._resolve_brackets(json_data['Instance']['ground_truth'].split('. Explanation')[0]):   # FIXME:字段名可能有变
            event = self._format(event)
            event = event.replace('arguments:', '')
            event_elements = self._resolve_comma(event)  # 因为后面会排序，所以每个pair的规整化需要提前进行
        ...
        y_pred = set()
        for event in self._resolve_brackets(predict.split('. Explanation')[0]):
            event = self._format(event)
            event = event.replace('arguments:', '')
            event_elements = self._resolve_comma(event)  # 因为后面会排序，所以每个pair的规整化需要提前进行
...
class MyEvaluatorEET(EvaluatorBase):
    def _init_metric(self):
        self.metric = MetricAcc()
    def _extract(self, json_data, predict: str):
        y_truth = json_data['Instance']['ground_truth'].split('. Explanation')[0]
        y_truth = self._format(y_truth)

        y_pred = self._format(predict.split('. Explanation')[0])
        return y_truth, y_pred
...
class MyEvaluatorEEA(EvaluatorBase):
    def _init_metric(self):
        self.metric = MetricF1()
    def _extract(self, json_data, predict: str):
        y_truth = set()
        for item in json_data['Instance']['ground_truth'].split('. Explanation')[0].split(';'):
            if ':' not in item:
                continue
            y_truth.add(self._format(item))
        
        y_pred = set()
        for item in self._format(predict.split('. Explanation')[0]).split(';'):
            if ':' not in item:
                continue
            y_pred.add(self._format(item))
